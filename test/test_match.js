...
const matchRegex = match.__get__('matchRegex');
...
describe('Match regular expression', () => {
  ...
  it('Should match', () => {
    ...
    const { match, captures, index } = matchRegex(line, pattern);
  ...
  it('Should destructure to undefined', () => {
    const { match, captures, index } = matchRegex(line, /nomatch/);
...
describe('Match specific patterns', () => {
  ...
  inputExpectations.forEach(obj => {
    it(`Should extract ordered data from '${obj.line}'`, () => {
      ...
      const out = match.postMatch(obj.line);
  ...
  inputExpectations.forEach(obj => {
    it(`Should extract ordered data when addtl notes are present from '${obj.line}'`, () => {
      ...
      const out = match.postMatch(obj.line + notes);
      assert.deepStrictEqual(out, expected);
    });
  });

  const inputExpectationsPrecedes = [
    // US precedes size.
  ...
  inputExpectationsPrecedes.forEach(obj => {
    it(`Should extract ordered data from '${obj.line}'`, () => {
      const expected = {
        size: obj.size,
        width: obj.width,
      ...
      const out = match.precedesMatch(obj.line);
      assert.deepStrictEqual(out, expected);
    });
  });
  ...
  inputExpectationsPrecedes.forEach(obj => {
    it(`Should extract ordered data when addtl notes are present from '${obj.line}'`, () => {
      ...
      const out = match.precedesMatch(obj.line + notes);
      assert.deepStrictEqual(out, expected);
    });
  });
